...
RSpec.describe GramsController, type: :controller do
  describe "grams#destroy action" do
    ...
    it "should return a 404 message if we cannot find a gram with the is that is specified" do
      user= FactoryBot.create(:user)
      sign_in user
      delete :destroy, params: { id: 'KAYAK' }
  ...
  describe "grams#update action" do
    it "shouldn't let a user who didn't create the gram update the gram" do
      ...
      user = FactoryBot.create(:user)
      sign_in user
      patch :update, params: { id: gram.id, gram: { message: 'Goodbye' } }
      expect(response).to have_http_status(:forbidden)
    end
    ...
    it "should have http 404 error if the gram cannot be found" do
      user = FactoryBot.create(:user)
      sign_in user
      patch :update, params: { id: "HANNAH", gram: { message: 'Changed' } }
      expect(response).to have_http_status(:not_found)
    end
  ...
  describe "grams#edit action" do
    ...
    it "should successfully show the edit from if the gram is found" do
      gram = FactoryBot.create(:gram)
      sign_in gram.user
      get :edit, params: { id: gram.id }
    ...
    it "should successfully return a 404 error message if the gram is not found" do
      user = FactoryBot.create(:user)
      sign_in user
      get :edit, params: { id: 'RACECAR' }
      expect(response).to have_http_status(:not_found)
    end
  end
